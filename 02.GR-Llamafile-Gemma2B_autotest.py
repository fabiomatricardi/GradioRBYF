...
with gr.Blocks(theme=gr.themes.Glass(), css=custom_css) as demo:
    ...
    with gr.Row(variant='compact'):
            with gr.Column(scale=1):            
                gr.Image(value='img/gemma.png', 
                        show_label = False, 
                        show_download_button = False, container = False)              
            with gr.Column(scale=4):
                gr.HTML("<center>"
                + "<h1>Revised Benchmark with You as a Feedback!</h1>"
                + "<h4>💎 Gemma2-2B - 4K context window with Ollama engine</h4></center>")
                with gr.Row(variant='compact'):
                    with gr.Column(scale=2):  
                        gr.Markdown("""#### Description:
                                *Run a prompt catalogue with 11 tasks*
                    ...
                    with gr.Column(scale=1): 
                        gr.Markdown("""#### Fixed tuning Parameters:
                                ```
                                temperature = 0.25
    ...
    with gr.Row(variant='compact'): # Progress status
            with gr.Column(scale=1):
                #btn_test = gr.Button(value='Start AutoTest', variant='secondary')
            ...
            with gr.Column(scale=4):          
                actual_progress = gr.Slider(0, len(tasks), 
                                            ...
                                            interactive=False)    
    with gr.Row(variant='compact'): # Button to start the test
            btn_test = gr.Button(value='Start AutoTest', variant='secondary')
    

    with gr.Row(variant='compact'): # KPI and ChatBot Area
            with gr.Column(scale=1):
            ...
            with gr.Column(scale=5):    
                myBOT =gr.Chatbot(history,type='messages',
                                  avatar_images=("./img/user.png","./img/assistant.png"),
                                  show_copy_button=True,render_markdown=True,
                                  line_breaks=True)      #autoscroll=True             
    ...
    def firstloop(a,b): #requires the a(actual_progress) and b(Chatbot) widget
        ...
        x = gr.Button(value='Start AutoTest', variant='secondary', visible=False)
        return taskName,b,x
    ...
    def startInference(a):
        ...
        ttFT = f"TimeToFristToken: N/A"
        a.append({"role":"assistant","content":generation})
        print(a[-1]['content'])
        return a, delta, speed_tokens, ttFT,total_tokens  
    
    btn_test.click(firstloop, inputs=[actual_progress,myBOT],
                    outputs=[act_task,myBOT,btn_test]).then(startInference,
...
"""
    ...
    btn_fbck.click(saveaLL, inputs=[myBOT,txt_fbck,actual_progress],
                    ...
                    [actual_progress,myBOT],[act_task,myBOT,btn_test]    
                    ).then(startInference,
                    [myBOT],[myBOT,txt_gentime,txt_speed,txt_ttft,txt_TOTtkns])
